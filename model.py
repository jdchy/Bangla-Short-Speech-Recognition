...
def get_recurrent_model():
	model = Sequential()
	model.add(LSTM(196, return_sequences=True, input_shape=input_shape))
	...
	model.add(TimeDistributed(Dense(64, activation='relu')))
	model.add(TimeDistributed(Dense(31, activation='relu')))
	model.add(Flatten())
	model.add(Dense(40, activation='softmax'))
	model.summary()
	model.compile(loss='categorical_crossentropy',optimizer='adam',metrics=['acc'])
...
history = model.fit(X, y, epochs=10, batch_size=32,shuffle=True,validation_split=0.2,callbacks=[checkpoint])
